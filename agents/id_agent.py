...
def run_id_agent(pages: list[str]) -> dict:
    """
    Takes a list of page texts that were classified as identity documents.
    Returns a dict with extracted identity fields.
    ...
    prompt = f"""You are an expert at extracting identity information from insurance claim documents.
...
"""
    ...
    try:
        ...
        if "403" in error_text or "Provider returned error" in error_text:
            print(
                f"[ID Agent] Primary model '{primary_model}' failed with provider error. "
    ...
    try:
        ...
        print(f"[ID Agent] Could not parse JSON response: {raw}")
